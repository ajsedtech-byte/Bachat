...
function formatSeller(s) {
  ...
  return {
    ...
    city: o.city,
    region: o.region,
    shop_images: Array.isArray(o.shopImages) ? o.shopImages : [],
    storefront_tagline: o.storefrontTagline || "",
    menu_note: o.menuNote || "",
    rating: o.rating,
...
function formatDeliveryPublic(d) {
  if (!d || !d.status || d.status === "none") {
    return {
      ...
      delivery_otp_sent_at: d?.deliveryOtpSentAt || null,
      delivery_otp_expires_at: d?.deliveryOtpExpiresAt || null,
      delivery_otp_verified_at: d?.deliveryOtpVerifiedAt || null,
      delivery_otp_required: Boolean(d?.deliveryOtpHash && !d?.deliveryOtpVerifiedAt),
